...
static HANDLE g_ProcessHandle = GetCurrentProcess();
static bool g_SymbolsInitialized = false;
static LONG (WINAPI *g_PreviousFilter)(EXCEPTION_POINTERS *) = nullptr;

static XString CKExceptionModuleNameFromAddress(const void *address) {
    HMODULE hModule = nullptr;
    if (!GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS | GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                            reinterpret_cast<LPCSTR>(address), &hModule) || !hModule)
        return "";

    return VxGetModuleFileName((INSTANCE_HANDLE)hModule);
}
...
static bool EnsureSymbolsInitialized() {
...
static void LogRegisterState(const CONTEXT &ctx) {
#if defined(_M_IX86)
    LogMessage(
...
static void LogExceptionHeader(EXCEPTION_RECORD *record) {
    XString moduleName = CKExceptionModuleNameFromAddress(record->ExceptionAddress);
    if (moduleName.Length() == 0)
        moduleName = "Unknown";

    LogMessage("[CK2_3D] CRASH: code=0x%08lX flags=0x%08lX at=0x%p module=%s thread=0x%04X",
               record->ExceptionCode, record->ExceptionFlags, record->ExceptionAddress, moduleName.CStr(),
               GetCurrentThreadId());
...
static void LogStackTrace(CONTEXT context) {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        XString moduleName = CKExceptionModuleNameFromAddress(reinterpret_cast<const void *>(addr));
        ...
        if (fileName && fileName[0]) {
            ...
            _snprintf_s(lineBuffer, sizeof(lineBuffer), _TRUNCATE,
                        ...
                        reinterpret_cast<void *>(addr), symbolName,
                        static_cast<unsigned long long>(displacement), moduleName.CStr());
        } else {
            _snprintf_s(lineBuffer, sizeof(lineBuffer), _TRUNCATE,
